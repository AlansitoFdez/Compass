...
El último paquete, y el más grande: 17 archivos, recorridos por el subdominio que cada uno protege (`core` → `tenders` → `ingestion` → `api`), no todos de golpe.

**Reorganización previa, decidida a mitad de paso.** `CLAUDE.md` documentaba como decisión explícita que los tests iban "planos en `tests/`, no al lado del fuente". Al llegar aquí se decidió lo contrario: si el código fuente se organiza por dominio, sus tests deben espejar esa misma división. Se crearon `tests/core/`, `tests/tenders/`, `tests/ingestion/`, `tests/api/`, se movieron los 16 archivos (`conftest.py` se queda en la raíz, compartido), se movió `fixtures/codice_entry_sample.xml` a `tests/ingestion/` (su único consumidor), y se corrigió la frase de `CLAUDE.md` para reflejar la nueva estructura. `testpaths = ["tests"]` en `pyproject.toml` no necesitó tocarse — pytest descubre recursivamente. Verificado con Docker levantado: los 73 tests siguieron pasando tras el movimiento (una vez corregida la ruta relativa de `fixtures/`, que tres tests calculaban con `Path(__file__).parent`).

**`tests/core`** — `conftest.py` (docstring de módulo y de la fixture `client`), `test_infra_connectivity.py` (dos tests, sin docstring ninguno) y `test_celery_app.py` (tres tests sin docstring, un comentario en español traducido).

**`tests/tenders`** — el que tenía más deuda: `test_tender_repository.py` con 11 docstrings ausentes (el peor archivo del baseline) más dos docstrings en español traducidos y varios comentarios; `test_tender_model.py` (un test, verificando que los enums vuelven como tipos reales tras el round-trip); `test_tender_schema.py` (dos tests) y `test_tender_vertical.py` (seis tests diminutos sobre funciones puras).

**`tests/ingestion`** — el paquete más grande, ocho archivos. `test_historical_loader.py` fue el peor de todo el proyecto (13 docstrings ausentes, contando los helpers `handler`/`_spy_download` anidados dentro de los tests, que sí llevan docstring por la decisión tomada al planificar). `test_daily_ingestion_task.py` fue el segundo peor (10, mismo patrón de helpers anidados). El resto (`test_atom_client.py`, `test_checkpoint.py`, `test_codice_codes.py`, `test_codice_parser.py`, `test_daily_ingestion.py`, `test_feed_reader.py`) tenían menos huecos pero varios comentarios y mensajes de `assert` en español, todos traducidos.

**`tests/api`** — `test_health.py` (sin ningún docstring, estaba en la lista de módulos sin documentar de `main.py`/`alembic/env.py`/`conftest.py`) y `test_tenders_endpoint.py` (cuatro tests más tres helpers; su docstring de módulo, que ya estaba bien escrito, cuenta el hallazgo real de la 1.10 sobre el `TestClient` corriendo en un event loop distinto al de `pytest-asyncio` — la misma familia de problema que `create_task_engine()` resuelve para Celery, encontrada aquí también).

**Verificación final, sobre el proyecto entero.** `uv run ruff check .`, `uv run ruff format --check .` y `uv run mypy` limpios; `uv run pytest` en **73 passed**, sin mover ni un test. Con este paso se cierran los seis paquetes de la subfase: `core`, `tenders`, `ingestion`, `api`+`main`, `alembic`, `tests`.
