...
- **Modelo de extracción: OpenRouter, nivel gratuito.** Sin coste, sin tarjeta, sin Google (descartado explícitamente). Candidatos a medir en la 3.4 con un golden set real, no a ciegas -- mismo método que decidió el modelo de embeddings en la 2.4. Escrito aquí en la planificación: DeepSeek R1 y Qwen3 Coder 480B (262K de contexto, de sobra para un pliego de 60-80 páginas). **Actualizado en la 3.4**: al llegar a esa subfase, ninguno de los dos seguía siendo gratuito en el catálogo de OpenRouter (`GET /api/v1/models`, verificado el 2026-09-09) -- el nivel gratuito había rotado por completo a otra generación de modelos. Candidatos reales medidos en la 3.4: `nvidia/nemotron-3-super-120b-a12b:free` y `nex-agi/nex-n2.5-pro:free`, los dos con `structured_outputs` soportado y mayor tamaño/contexto del catálogo gratuito vigente en esa fecha. La API de OpenRouter es compatible con el formato de OpenAI vía REST normal -- no hizo falta SDK nuevo, `httpx2` (ya es dependencia) bastó.
